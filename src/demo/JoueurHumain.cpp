...
Joueurhumain::Joueurhumain(std::string n,int nbPions, int sc):F_Joueur(n,nbPions,sc),passe_tour(false){}
...
/// @todo retour à la case départ si la case d'arrivée est occupée
    ...
    case REJOUER:   {pions[0].setPosition(position);
                    case_courant.ajoutPion(pions[0]); jouer();} break;

    case PASSE:  {pions[0].setPosition(position);passe_tour=true;} break;

    default : pions[0].setPosition(position); break ;
